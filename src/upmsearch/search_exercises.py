    ...
    for instant in range(calculate_makespan(task_duration)):
        ...
        for span_task in range(len(chromosome)):
            if chromosome[span_task] != -1:
                if (instant > chromosome[span_task]) & (
                        instant <= (chromosome[span_task] + task_duration[span_task])):
...
def branch_and_bound(disponibilidad, cumpleDependencias, **kwargs):
    for i in range(calculate_makespan(task_duration=kwargs['task_duration'])):
        ...
        cumpleDependencias = checkResources(disponibilidad, cumpleDependencias, task_resource=kwargs['task_resource'], resources=kwargs['resources'])
